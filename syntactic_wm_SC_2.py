    ...
    def update_temp_activation(self):
        ...
        for n in range(self.n_role_neurons): #parent
            for i in range(self.n_role_neurons): #child
                if n == 0 and i == 2:
                    self.role_neuron_dict[str(i)].temp_update_activate(self.cc_connectivity_factor*self.role_neuron_dict[str(n)].act*self.cc_conn_dict[str(n)+str(i)].connection*1.7)
                    # also update temporary store matrix
                    self.temp_act_mat[n,i] = self.cc_connectivity_factor*self.role_neuron_dict[str(n)].act*self.cc_conn_dict[str(n)+str(i)].connection*1.7
                elif n == 0 and i == 1:
                    self.role_neuron_dict[str(i)].temp_update_activate(self.cc_connectivity_factor*self.role_neuron_dict[str(n)].act*self.cc_conn_dict[str(n)+str(i)].connection*0.3)
                    # also update temporary store matrix
                    self.temp_act_mat[n,i] = self.cc_connectivity_factor*self.role_neuron_dict[str(n)].act*self.cc_conn_dict[str(n)+str(i)].connection*0.3
                else:        
